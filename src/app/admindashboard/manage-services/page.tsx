...
export default function ManageServices() {
    ...
    const handleContentChange = (id: string, field: "subheader" | "description", value: string) => {
        setNewService((prev) => ({
            ...prev,
            contents: prev.contents.map((content) => (content.id === id ? { ...content, [field]: value } : content)),
        }));
    };
    ...
    const handleFormatting = (contentId: string, action: FormattingAction, isEdit: boolean = false) => {
        ...
        if (isEdit) {
            ...
            setEditService((prev) => ({
                ...prev,
                contents: prev.contents.map((c) => (c.id === contentId ? { ...c, description: newValue } : c)),
            }));
        } else {
    ...
    const handleAddService = async (e: React.FormEvent) => {
        ...
        const hasEmptyContent = newService.contents.some((content) => !content.subheader.trim() || !content.description.trim());
    ...
    const handleUpdateService = async (e: React.FormEvent) => {
        ...
        const hasEmptyContent = editService.contents.some((content) => !content.subheader.trim() || !content.description.trim());
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setServices(services.map((service) => (service._id === selectedService?._id ? response.data.service : service)));
        ...
        <div className={`flex min-h-screen ${isDarkMode ? "bg-gray-900" : "bg-gray-50"}`}>
            ...
            <div className="flex-1 flex flex-col ml-64">
                ...
                <div className={`px-8 py-4 flex items-center justify-between fixed top-0 right-0 left-64 z-10 ${isDarkMode ? "bg-gray-900" : "bg-gray-50"} shadow-sm`}>
                    ...
                    <div className="flex-shrink-0">
                        <h1 className={`text-2xl font-bold ${isDarkMode ? "text-white" : "text-black"}`}>Manage Services</h1>
                        ...
                        <div className={`p-6 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-white"} shadow-lg`}>
                                ...
                                <table className="min-w-full">
                                    ...
                                    <tbody className={`${isDarkMode ? "text-gray-300" : "text-gray-700"} divide-y ${isDarkMode ? "divide-gray-700" : "divide-gray-200"}`}>
                                        {getPaginatedServices().map((service) => (
                                            <tr key={service._id} className={`${isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-50"}`}>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center space-x-3">
                                                        <div className={`w-8 h-8 flex items-center justify-center rounded-lg ${isDarkMode ? "bg-gray-700" : "bg-gray-100"}`}>
                            ...
                            <div className="mt-6 flex justify-between items-center">
                                <div className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                                    Showing {Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, services.length)} to {Math.min(currentPage * ITEMS_PER_PAGE, services.length)} of{" "}
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 backdrop-blur-sm bg-white/30 dark:bg-gray-900/30 flex items-center justify-center z-50">
                    ...
                    >
                        <h3 className={`text-xl font-semibold mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Confirm Delete</h3>
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <button onClick={confirmDelete} disabled={loading} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 disabled:opacity-50">
            ...
            {isAddServiceModalOpen && (
                <div className="fixed inset-0 backdrop-blur-sm bg-white/30 dark:bg-gray-900/30 flex items-center justify-center z-50">
                    <div
                        className={`${isDarkMode ? "bg-gray-800/95" : "bg-white/95"} rounded-lg p-8 w-full max-w-4xl relative shadow-xl backdrop-blur-sm border ${
                            isDarkMode ? "border-gray-700" : "border-gray-200"
                        } max-h-[90vh] overflow-y-auto`}
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h3 className={`text-2xl font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Add New Service</h3>
                            <button
                                onClick={() => {
                                    setIsAddServiceModalOpen(false);
                                    setNewService({
                        ...
                        <form onSubmit={handleAddService} className="space-y-6">
                            ...
                            <div className="relative">
                                <label className={`block text-sm font-medium mb-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>Select Icon</label>
                                <button
                                    type="button"
                                    onClick={() => setIsIconDropdownOpen(!isIconDropdownOpen)}
                                    className={`w-full px-4 py-3 rounded-lg border ${
                                        isDarkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-700"
                                    } focus:outline-none focus:border-pink-500 ${formErrors.icon ? "border-red-500" : ""} flex items-center justify-between`}
                                >
                                ...
                                {isIconDropdownOpen && (
                                    ...
                                    >
                                        {["Screening & Detection", "Diagnosis", "Treatment", "Support Services", "Prevention & Wellness"].map((category) => (
                                            <div key={category}>
                                                <div className={`px-4 py-2 text-sm font-semibold ${isDarkMode ? "bg-gray-800 text-gray-300" : "bg-gray-50 text-gray-700"}`}>
                                                    {category}
                                                </div>
                                                <div className="p-2">
                                                    {availableIcons
                                ...
                                {formErrors.icon && <p className="text-red-500 text-sm mt-1">{formErrors.icon}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-4 py-3 rounded-lg border ${
                                        isDarkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-700"
                                    } focus:outline-none focus:border-pink-500 ${formErrors.name ? "border-red-500" : ""}`}
                            ...
                            <div className="space-y-6">
                                <div className="flex justify-between items-center">
                                    <h4 className={`text-lg font-medium ${isDarkMode ? "text-white" : "text-gray-900"}`}>Content Sections</h4>
                                    <button type="button" onClick={handleAddContent} className="text-pink-600 hover:text-pink-700 flex items-center space-x-1">
                                ...
                                {newService.contents.map((content, index) => (
                                    <div key={content.id} className="relative p-6 border rounded-lg space-y-4 bg-gray-50 dark:bg-gray-700/50">
                                        ...
                                        <div className="absolute -top-3 left-4 px-2 py-1 rounded bg-pink-500 text-white text-sm">Section {index + 1}</div>
                                        ...
                                        {/* Subheader */}
                                        <div>
                                            <label className={`block text-sm font-medium mb-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>Subheader</label>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            ...
                                            <div className={`border rounded-lg ${isDarkMode ? "border-gray-600" : "border-gray-300"}`}>
                                                {/* Formatting Toolbar */}
                                                <div
                                                    className={`flex items-center gap-1 p-2 border-b ${isDarkMode ? "border-gray-600 bg-gray-700" : "border-gray-200 bg-gray-50"}`}
                                                >
                                                    <button
                                                        type="button"
                                                        onClick={() => handleFormatting(content.id, "bold")}
                                                        className={`p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-600 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
                                                        title="Bold"
                                                    >
                            ...
                            <div className="flex justify-end space-x-4 pt-6">
                                <button
                                    ...
                                    className={`px-6 py-3 rounded-lg ${isDarkMode ? "bg-gray-700 text-gray-300 hover:bg-gray-600" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
                                >
                                    Cancel
                                </button>
                                <button
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 backdrop-blur-sm bg-white/30 dark:bg-gray-900/30 flex items-center justify-center z-50">
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h3 className={`text-2xl font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>Edit Service</h3>
                            <button
                                onClick={() => {
                                    setIsEditModalOpen(false);
                                    setSelectedService(null);
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    setEditService({
                                                        ...editService,
                                                        contents: editService.contents.map((c) => (c.id === content.id ? { ...c, subheader: e.target.value } : c)),
                                                    });
                                                }}
                                                className={`w-full px-4 py-2 rounded-lg border ${
                                                    isDarkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-700"
                                            ...
                                            <div className={`border rounded-lg ${isDarkMode ? "border-gray-600" : "border-gray-300"}`}>
                                                ...
                                                <textarea
                                                    ...
                                                    onChange={(e) => {
                                                        setEditService({
                                                            ...editService,
                                                            contents: editService.contents.map((c) => (c.id === content.id ? { ...c, description: e.target.value } : c)),
                                                        });
                                                    }}
                                                    rows={6}
                                                    className={`w-full px-4 py-2 rounded-b-lg ${
